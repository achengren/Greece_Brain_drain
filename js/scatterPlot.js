function renderQualityScatterPlot(rawData) {
    ...
    controlLabel.textContent = '指标';
    control.appendChild(controlLabel);

    const wrap = document.createElement('div');
    wrap.className = 'custom-select quality-metric-wrap';
    const trigger = document.createElement('button');
    trigger.className = 'cs-trigger';
    trigger.type = 'button';
    trigger.innerHTML = `<span class="cs-value">${selectedMetric.label}</span><span class="cs-arrow"></span>`;
    const menu = document.createElement('ul');
    menu.className = 'cs-menu';
    metrics.forEach(metric => {
        const li = document.createElement('li');
        li.className = 'cs-option' + (metric.key === selectedMetric.key ? ' selected' : '');
        li.dataset.value = metric.key;
        li.textContent = metric.label;
        li.addEventListener('click', (e) => {
            e.stopPropagation();
            menu.querySelectorAll('.cs-option').forEach(o => o.classList.remove('selected'));
            li.classList.add('selected');
            trigger.querySelector('.cs-value').textContent = metric.label;
            trigger.classList.remove('open');
            menu.classList.remove('open');
            menu.style.maxHeight = '0';
            selectedMetric = metrics.find(d => d.key === metric.key) || selectedMetric;
            draw();
        });
        menu.appendChild(li);
    });
    trigger.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.cs-menu.open').forEach(el => { el.classList.remove('open'); el.style.maxHeight = '0'; });
        if (!isOpen) {
            trigger.classList.add('open');
            menu.classList.add('open');
            menu.style.maxHeight = Math.min(menu.scrollHeight, 240) + 'px';
        }
    });
