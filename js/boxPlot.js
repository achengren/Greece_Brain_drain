function renderQualityBoxPlot(rawData) {
    ...
    function updateSummary() {
        ...
        summary.innerHTML = `
            <div class="quality-stat">
                <span>中位排名（越低越靠前）</span>
                <strong>海外 ${formatPercent(overseas.raw.median)}，本土 ${formatPercent(domestic.raw.median)}</strong>
                <em>海外领先 ${medianGap.toFixed(1)} pp</em>
            </div>
            <div class="quality-stat">
                <span>Top 1% 占比</span>
                <strong>海外 ${formatShare(top1.overseasShare)} / 本土 ${formatShare(top1.domesticShare)}</strong>
            </div>
            <div class="quality-stat">
                <span>Top 10% 占比</span>
                <strong>海外 ${formatShare(top10.overseasShare)} / 本土 ${formatShare(top10.domesticShare)}</strong>
            </div>
        `;
    }
    ...
    function drawChart() {
        ...
        const height = Math.max(330, Math.min(400, width * 0.40));
        const margin = { top: 32, right: 28, bottom: 42, left: 72 };
